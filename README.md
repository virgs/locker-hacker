...
* **Unified game menu** — stats, settings, and help now live in one accordion-based menu under a single `Menu` title; `Play again` reopens it on the game-stats section, the help icon opens it directly on how-to-play, the accordion headers use left-aligned icons, the navbar app icon is now decorative instead of opening a separate modal, and the build label/reset gesture lives in the shared footer
...
* **Radial dot annotations while solving** — after your first valid guess starts the round, long-press a dot to open a circular note menu, then point toward `clear`, `eliminate`, `all`, or any numbered position `1..N` and release; the menu is now roomier on both desktop and mobile, and on phones it still keeps a wider safety margin from the screen edge, pushes the numbered options farther from the center with larger labels, flanks the top arc with action icons, snaps the pointed choice into the center for clearer release feedback, lifts above the navbar, history rail, and footer so annotation choices stay visible, and avoids the mobile browser magnifier/loupe conflict from repeated taps
* **Browser context menu suppressed** — right-click and long-press context menus are disabled across the app so they do not interrupt pattern input or annotation gestures
