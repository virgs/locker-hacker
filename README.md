...
* **Radial dot annotations while solving** — after your first valid guess starts the round, double-click a dot on desktop or long-press it on touch screens to open a circular note menu, then point toward `clear`, `eliminate`, `all`, or any numbered position `1..N` and release; on phones the menu expands to a much larger footprint with bigger touch targets, keeps a wider safety margin from the screen edge, pushes the numbered options farther from the center with larger labels, flanks the top arc with action icons, snaps the pointed choice into the center for clearer release feedback, lifts above the navbar, history rail, and footer so annotation choices stay visible, and now avoids the mobile browser magnifier/loupe conflict from repeated taps
