...
1. [x] Use the react-bootstrap modal component to reveal the code instead of creating a custom one. This will ensure better accessibility and consistency with the rest of the UI. Also, I created the "ai-inference-rules.md" file to explain the AI's logic in a more user-friendly way. I asked an ai agent to come up with a code to that implementation ai/summary-builder.ts, but it came up with a lot of repetition of what we already have in place. So, improve it. Get rid of the repetition, make it more concise, reuse what we already have, clean the new code, spread each class into a new ones, rename the file if you have to, unit test them all. Also, make sure to update the README.md to mention where to find the "ai-inference-rules.md" file for reference.
