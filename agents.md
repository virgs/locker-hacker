...
1. [ ] I feel like some app.tsx component states belong together to something like GameContext. For example, the "number of players", "level", "code" and "game key" are  related to the game configuration and don't change during the game. Maybe path, pathHistory and phase can also be combined into a new Type. So, I think it would be better to create these types to hold these states and provides them to the components that need them. This way, we can avoid prop drilling and make the code more organized. Also, get rid of the "new game"/"play button". Keep the "Give up" though when the game is running. The Play/New game are not needed. The game is considered to be running when the user has made at least one move and the game is not in the "reveal" phase. So, there's no need for the play button. Ask questions if you didnt understand something or if you think this is not a good idea.
2. [ ] Different players should have different colors in the pattern lock and in the dropdown items (the pattern locks in the history should still have the same color) (use bootstrap colors: primary, success, warning, info).
3. [ ] In small screens the nav items should only be their icons. Both dropdown links and also the play button. Regardless of the screen size, the play/reveal button should always be horizontally centralized.
4. [ ] On multiplayer games (aka games with more than one player), there should be a 3 seconds long modal announcing whose turn it is after a player makes a move, the turn should switch to the next player. The announcement should say something like "Player 2 turn" or something similar with the same meaning and the "Player X" should have the color of the player whose turn it is.
5. [ ] After every one player game, update and show stats (number of wins, losses, time taken for each game, etc.), then show (avg win %, time with deciseconds) and total time. Stats should be stored in local storage so they persist across sessions. When the user clicks the game icon in the nav bar, a modal with these information in a table is shown. If you hold the icon for too long, these game stats should be clear from the local storage and the modal should show "No stats available. Play some games to see stats here!".
6. [ ] Add a footer that tells what's the current code length, the current level and total time with the seconds precision. This should be visible in all screens.
7. [ ] When implicitly adding a new dot to the current path like going from 1 to 3 without going through 2, the new dot should show the animation dots show when are selected.
