...
1. [ ] Instead of the reveal modal automatically disappearing after a few seconds, add two buttons "Dismiss" and "New Game" (or similar more intuitive namings) that the user can click. The "dismiss"  dismisses the modal and the game screen should be restored, but with the main pattern lock disabled so no player can play anymore. The pattern lock history should still be visible and scrollable. Once the new game button is hit, the game should go to the initial state where the "players", "level" and "play" buttons are enabled.
2. [ ] Different players should have different colors (in the pattern lock only) (use bootstrap colors: primary, success, warning, info)
3. [ ] In small screens the nav items should only be their icons. Both dropdown and also the play button. Regardless of the screen size, the play/reveal button should always be horizontally centralized.
3. [ ] On multiplayer games (aka games with more than one player), there should be a 3 seconds long modal announcing whose turn it is After the player makes a move, the turn should switch to the other player. The modal should have the color of the player whose turn it is.
4. [ ] After every one player game, update and show stats (number of wins, losses, time taken for each game, etc.), then show (avg win %, time) and total time. Stats should be stored in local storage so they persist across sessions. When the user clicks the game icon in the nav bar, a modal with these information in a table is shown. If you hold the icon for too long, these game stats should be clear from the local storage and the modal should show "No stats available. Play some games to see stats here!".
