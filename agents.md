...
1. [ ] The give up button should give place to a "Hint" dropdown (adjust the icon accordingly) The dropdown should have two options: "Get a hint" (Or eliminate dot, something like this and a proper icon) and "Give up". The "Give up" option will allow the user to end the current game and see the solution as the current button does, while the "Get a hint" option will randomly select a dot that is not in the secret code and mark a red (bootstrap danger) behind it in the Pattern lock. It's a helpful clue to assist the player in solving the puzzle. The number of hints per game should also be displayed in the stats table modal (use toFixed(1) to show it and a nice icon too). This change will enhance the user experience by offering more choices and support during gameplay, making it more engaging and enjoyable.
