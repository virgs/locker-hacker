...
1. [x]  The stats modal and help modal should merge into a single menu model with accordion sections or tabs (whichever you prefer) for stats, help, and settings. This will simplify the UI and make it easier to navigate. The stats section will be what the stats modal currently shows, the help section will be what the help modal currently shows, and the settings section will have one switch "Enable annotations" that enables/disables annotations. The settings should be persisted in local storage so that it remembers the user's preference across sessions. The stats and help sections can be static content, while the settings section will have interactive elements. This will also make it easier to add new sections in the future if needed. Clicking the navbar icon won't do anything anymore. But after each game and clicking the "Play Again" button, the menu will automatically open to show the stats (accordeon or tab). When clicking on the help icon in the navbar, it will open the menu to the help section (accordeon or tab).
