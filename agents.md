...
1. [x] I see these hardcoded colors spread in the codebase (    bull: "#22c55e",
   cow:  "#eab308",
   miss: "#6b7280",
   ), we should move them to a central theme file to maintain consistency and make it easier to update colors in the future. Not only these, but every piece of code that is used multiple times and has a specific meaning should be moved to a central file. For example, if we have a specific color for bull or players, or dots..., we can define it in a theme file as `BULL_COLOR` and use that constant throughout the codebase instead of hardcoding the color value. This way, if we want to change the color scheme, we can do it in one place instead of searching through the entire codebase for hardcoded values. Also, instead of representing the bulls and cows as color coded circles we can represent them as color coded shapes. Let's keep the same colors, but instead of using circles, we can use different shapes to represent bulls and cows. For example, we can use a plus sign for bulls and a minus sign for cows and small circle for miss. This way, even if someone is colorblind, they can still differentiate between the two by looking at the shape. This would enhance accessibility and make the game more inclusive for all players. Make sure you update the feedback history and the help menu.